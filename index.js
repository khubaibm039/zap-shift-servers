...
// const decoded = Buffer.from(process.env.FB_PRIVATE_KEY, 'base64').toString('utf8')
...
// ---------------------------------------------------------------
// Firebase admin init
// Guarded with `if (!admin.apps.length)` because on Vercel a warm
// serverless instance can re-import this module without a fresh
// process, and calling initializeApp() twice throws.
// ---------------------------------------------------------------
if (!getApps().length) {
    initializeApp({
        credential: cert({
...
// On Vercel, a "warm" serverless instance keeps its module scope
// alive between requests, so we cache the connection promise on
// `global` instead of reconnecting (and instead of connecting
// inside a one-time run() that also called app.listen()).
// ---------------------------------------------------------------
const uri = `mongodb+srv://${process.env.DB_USER}:${process.env.DB_PASSWORD}@cluster0.ag6bkre.mongodb.net/?appName=Cluster0`;
...
// Attach collections to every request once the client is ready.
// This replaces the old pattern of defining routes *inside* run()
// after client.connect() resolved — that pattern only works for a
// server that stays up; on Vercel each request is a fresh function
// call, so routes must exist immediately and just await the DB.
app.use(async (req, res, next) => {
    try {
        ...
        req.db = {
            ...
            trackingsCollection: database.collection("trackings"),
        };
        next();
    } catch (err) {
...
app.get("/parcels", async (req, res) => {
    try {
        const { parcelsCollection } = req.db;
        const query = {};
        const { email, deliveryStatus } = req.query;
...
app.post("/parcels", async (req, res) => {
    try {
        const { parcelsCollection, trackingsCollection } = req.db;
        const parcel = req.body;
        const trackingId = generateTrackingId();
        parcel.createAt = new Date();
        parcel.trackingId = trackingId;
...
app.patch("/parcels/:id", async (req, res) => {
    const { parcelsCollection, riderCollection, trackingsCollection } = req.db;
    const { riderId, riderName, riderEmail, trackingId } = req.body;
    const id = req.params.id;
    ...
    res.send({ parcelsResult, riderResult });
});
...
app.patch("/payment-success", async (req, res) => {
    try {
        const { paymentCollection, parcelsCollection, trackingsCollection } =
            req.db;
        const sessionId = req.query.session_id;
...
app.get("/payments", verifyFBToken, async (req, res) => {
    try {
        const { paymentCollection } = req.db;
        const { email } = req.query;
        const query = {};
        if (email) {
            query.customerEmail = email;
...
app.patch("/riders/:id", verifyFBToken, verifyRider, async (req, res) => {
    const { riderCollection, userCollection } = req.db;
    const status = req.body.status;
    const id = req.params.id;
    const result = await riderCollection.updateOne(
        { _id: new ObjectId(id) },
...
// ---------------------------------------------------------------
app.get("/trackings/:trackingId/logs", async (req, res) => {
    const { trackingsCollection } = req.db;
...
// On Vercel: the platform imports `module.exports` and calls it as
// a request handler — it never runs this branch, so app.listen()
// here would otherwise be dead weight (and cause "port already in
// use" noise in serverless logs) if left unguarded.
// ---------------------------------------------------------------
